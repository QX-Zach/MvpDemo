...
public class HoYoImgHelper {
    ...
    //获取图片缩略图并保存本地
    public static ScaleBitmap getSmallBitmap(Context context, String filePath) {
        ScaleBitmap result = new ScaleBitmap();
        String filename = filePath.substring(filePath.lastIndexOf("/") + 1);
        if (!filename.startsWith("hoyochache_")) {
            filename = "hoyochache_" + filename;
            if (context != null) {
                File tempParentDir = new File(Environment.getExternalStorageDirectory().getPath() + "/HoYoChache/");
                ...
                File tempFile = new File(Environment.getExternalStorageDirectory().getPath() + "/HoYoChache/" + filename);
